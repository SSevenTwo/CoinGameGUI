...
public class AddPlayerBtnListener implements ActionListener {
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if(!checkIfValidPlayerIDName(newPlayerId)) {
			return;
		};

		String newPlayerName = JOptionPane.showInputDialog("Please enter the player name:");
		
		if(!checkIfValidPlayerIDName(newPlayerName)) {
			return;
		};
	...
	private int getInitialPointsDialog() {
		...
		try {
			...
			JOptionPane.showMessageDialog(mainFrame, "You must enter a number!", "Error", JOptionPane.ERROR_MESSAGE);
			newPlayerPoints = -1;
		}
		;
	...
	private boolean checkIfValidPlayerIDName(String newPlayerIdOrName) {
		if (newPlayerIdOrName == null) {
			return false;
		} else if (newPlayerIdOrName.trim().equals("")) {
			JOptionPane.showMessageDialog(mainFrame, "Invalid player ID or Name!", "Error", JOptionPane.ERROR_MESSAGE);
			return false;
		}
		return true;
	}
